...
public class WebController {
    ...
    public String showUserList(@RequestParam(name = "email", required = false, defaultValue = "") String email,
                               ...
                               @RequestParam(name = "id", required = false, defaultValue = "") Long id, Model model) {
       List<User> users = repository.findAll();

        //List<User> users = repository.getUsersSortedById();
        //List<User> users = repository.getTop2UsersLastInsertedFromDatabase();
        //List<User> users = repository.getUsersSortedByNameDesc();
        //List<User> users=repository.getUsersNameEqualSurname();
        //List<User>users=repository.getUsersWhichEmailContains();
        //List<User> users=repository.getUsersNameDistinct();
...
//        }
//            if(!name.isEmpty()){
//            users=repository.findByNameAndSurnameEquals(name);
//        }
